 ...
 */
const genTestUserSig = (userID: string): { sdkAppId: number; userSig: string } => {
  /**
  ...
  const SDKAPPID: number = PLACEHOLDER;
  ...
  const SECRETKEY: string = "PLACEHOLDER";

  const generator = new LibGenerateTestUserSig(SDKAPPID, SECRETKEY, EXPIRETIME);
  const userSig: string = generator.genTestUserSig(userID);
  return {
    sdkAppId: SDKAPPID,
    userSig: userSig,
  };
